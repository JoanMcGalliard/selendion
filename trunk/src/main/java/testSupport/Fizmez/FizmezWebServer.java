...
public class FizmezWebServer extends ErrorHandler implements Runnable
{
	...
	private String configFile="./conf/httpd.conf";
	...
	FizmezWebServer()
	{
		// Create new ErrorHandler and announce birth
		this.setLoggingLevel(NOTICE);
		//this.setLoggingLevel(ErrorHandler.DEBUG);
		...
		while(running)
		{
			...
			while(stillServingRequests)
			{
				...
				if(!headerError)
				{
					...
					// Other Browser?
					else if (acceptString.indexOf("*/*")>=0 || acceptString.indexOf("text/html")>=0 || acceptString.length()==0)
					{
					...
					if (requestUri.indexOf(".cgi")>=0)
					{
						...
						try
						{
							BufferedReader execBufferedReader=new BufferedReader(new FileReader(configFile));
